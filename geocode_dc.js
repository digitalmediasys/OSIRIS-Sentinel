...
function geocode() {
  ...
  https.get(url, { headers: { 'User-Agent': 'OSIRIS-DC-Geocoder/1.0 (your-email@example.com)' } }, res => {
    let d = '';
    res.on('data', c => d += c);
    res.on('end', () => {
      try {
        ...
        if (json.length > 0) {
          results.push({ address, lat: parseFloat(json[0].lat), lng: parseFloat(json[0].lon) });
          console.log(i + 1, 'OK', address, result = json[0].lat + ',' + json[0].lon);
        } else {
          console.log(i + 1, 'NORESULT', address);
      ...
      i += 1;
      setTimeout(geocode, 1200);
    });
  }).on('error', e => { console.error(i+1, 'ERR', e.message, address); results.push({ address, lat: null, lng: null }); i += 1; setTimeout(geocode, 1200); });
}
geocode();
