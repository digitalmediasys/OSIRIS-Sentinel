...
const DEFAULT_MAP_STYLE = {
  ...
  layers: [
    { id: 'dark-base', type: 'raster', source: 'dark' },
  ],
};
...
function OsirisMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData }: OsirisMapProps) {
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      center: [-98.0, 39.0], zoom: 3.8, minZoom: 1.5, maxZoom: 18,
    ...
    map.on('load', () => {
      ...
      // USA Traffic Cameras — outer glow ring
      map.addLayer({ id: 'traffic-glow', type: 'circle', source: 'traffic-cameras', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,4, 5,7, 10,12, 14,18],
        'circle-color': '#FF8C00', 'circle-opacity': 0.08, 'circle-blur': 1,
      }});
      // USA Traffic Cameras — main dot
      map.addLayer({ id: 'traffic-dots', type: 'circle', source: 'traffic-cameras', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,2.5, 5,4, 10,7, 14,10],
        'circle-color': '#FF8C00', 'circle-opacity': 0.8,
        'circle-stroke-width': 2, 'circle-stroke-color': '#FF8C00', 'circle-stroke-opacity': 0.5,
      }});
      // USA Traffic Cameras — labels at zoom 9+
      map.addLayer({ id: 'traffic-label', type: 'symbol', source: 'traffic-cameras', minzoom: 9, layout: {
      ...
      map.addLayer({ id: 'home-depot-trucks-dots', type: 'circle', source: 'home-depot-trucks', paint: {
      ...
      map.addLayer({ id: 'home-depot-trucks-label', type: 'symbol', source: 'home-depot-trucks', minzoom: 8, layout: {
        'text-field': ['concat', ['get', 'carrier'], '\n', ['get', 'asn']],
        'text-size': 9,
    ...
    map.on('click', 'home-depot-trucks-dots', e => {
      ...
      const p = e.features[0].properties as any;
      const coords = (e.features[0].geometry as any).coordinates;
      popup(coords, `<div style="${pStyle}border:1px solid rgba(124,77,255,0.25);">
        <div style="color:#B39DDB;font-size:13px;font-weight:700;margin-bottom:8px;">${p.name}</div>
        <div style="font-size:10px;color:#E8E6E0;margin-bottom:6px;">${p.carrier} — ${p.city}, ${p.state}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:9px;">
          <div><span style="color:#5C5A54;">CARRIER</span><br/><span style="color:#B39DDB;">${p.carrier}</span></div>
  ...
  useEffect(() => {
    if (!mapReady) return;
    setGeo('home-depot-trucks', activeLayers.home_depot_trucks && data.home_depot_trucks ? data.home_depot_trucks.map((location: any) => ({
      type: 'Feature',
      geometry: { type: 'Point', coordinates: [location.lng, location.lat] },
      properties: {
        name: location.name,
  ...
  useEffect(() => {
    if (!mapReady || !activeLayers.traffic_cameras) return;
    // Fetch USA traffic cameras from API
    const fetchTrafficCameras = async () => {
      try {
        const response = await fetch('/api/traffic-cameras');
        if (!response.ok) throw new Error('Failed to fetch traffic cameras');
        ...
        setGeo('traffic-cameras', features);
      } catch (error) {
        console.error('Error fetching traffic cameras:', error);
        setGeo('traffic-cameras', []);
      }
    };
    fetchTrafficCameras();
  }, [mapReady, activeLayers.traffic_cameras, setGeo]);
  ...
  useEffect(() => {
    ...
    try {
      if (mapStyle === 'satellite') {
        // Add satellite raster tiles using the ArcGIS World Imagery service
        if (!map.getSource('satellite-tiles')) {
          ...
          map.addLayer(
            { id: 'satellite-layer', type: 'raster', source: 'satellite-tiles', paint: { 'raster-opacity': 0.85 } },
            'day-night-fill'
          );
        } else {
