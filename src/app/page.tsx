...
export default function Dashboard() {
  ...
  const [mapView, setMapView] = useState({ zoom: 3.8, latitude: 39 });
  ...
  const handleEntityClick = useCallback((entity: any) => {
    if (entity?.type === 'cctv' || entity?.type === 'traffic_camera') setActiveCamera(entity);
  ...
  return (
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
      ...
      <AnimatePresence>
        {showSplash && (
          ...
          >
            ...
            {/* ── SENTINEL title — letter-by-letter stagger ── */}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }} className="status-bar-desktop absolute top-3 right-3 md:top-4 md:right-5 z-[200] pointer-events-none flex items-center gap-1.5 md:gap-3 text-[9px] md:text-[10px] font-mono tracking-widest text-[var(--text-muted)]">
        ...
        <div className="pointer-events-auto hover:opacity-80 transition-opacity ml-1 flex items-center">
          <span className="px-3 py-1 rounded-sm border border-[var(--gold-primary)]/40 bg-[var(--gold-primary)]/10 text-[var(--gold-primary)] text-[11px] font-bold tracking-[0.2em]">SENTINEL</span>
        </div>
      </motion.div>
      ...
      {isMobile && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }} className="absolute top-3 right-3 z-[200] pointer-events-auto flex items-center gap-2">
          <div className="glass-panel px-2 py-1 flex items-center gap-1.5 text-[7px] font-mono tracking-widest hover:opacity-80 transition-opacity border-[var(--gold-primary)]/40 bg-[var(--gold-primary)]/10">
            <div className="w-1 h-1 rounded-full bg-[var(--gold-primary)] animate-osiris-pulse" />
            <span className="text-[var(--gold-primary)] font-bold">SENTINEL</span>
          </div>
        </motion.div>
      )}
